...
def download_file(url, output_dir, preserve_structure=False):
    ...
    try:
        ...
        if preserve_structure:
            ...
            path_parts = url.split('Bacteria/')[1].split('/')
            if len(path_parts) > 1:
...
def main():
    ...
    parser.add_argument('--output-dir', '-o', default='./uniprot_bacteria', help='Output directory')
    ...
    # Base URL for bacterial reference proteomes
    base_url = "https://ftp.uniprot.org/pub/databases/uniprot/knowledgebase/reference_proteomes/Bacteria/"
